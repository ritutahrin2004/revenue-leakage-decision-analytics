-- Analysis: Weekly inventory stress (proxy) and revenue exposure
-- Stress definition: product-week with >=3 days where net_stock_in <= 0
...
SELECT
  w.week_start,
  SUM(w.product_revenue) AS total_revenue,
  SUM(CASE WHEN s.stressed_days >= 3 THEN w.product_revenue ELSE 0 END) AS revenue_exposed_to_inventory_risk,
  CASE
